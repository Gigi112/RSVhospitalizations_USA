...
for(k in 1:n.group){  
  ...
  for (p in 1:n.year) { 
    # epi-year effects
    epi[p,j,k,n] ~ dnorm(epi_mean[p,j,k,n], tau1) # random slope
    epi_mean[p,j,k,n]<- xi_epi[p]*(gamma_epi[k]+ omega_epi[j]+ zeta_epi[n])
...
zeta_epi[n] ~ dnorm(0,tau3)
}

for(k in 1:n.group){
gamma_flu[k] ~ dnorm(0,tau4)
gamma[k] ~ dnorm(0, tau5)
gamma_epi[k] ~ dnorm(0,tau6)
}

for(j in 1:n.age){
omega[j] ~ dnorm(0, tau7)
omega_flu[j] ~ dnorm(0,tau8)
omega_epi[j] ~ dnorm(0,tau9)
}
...
for (p in 1:n.year) { 
xi_flu[p] ~ dnorm(0,tau11)
xi_epi[p] ~ dnorm(0,tau12)
}
  eta ~ dnorm(0, 0.0001)
  ...
  tau10 ~ dgamma(0.01, 0.01)
  tau11 ~ dgamma(0.01, 0.01)
  tau12 ~ dgamma(0.01, 0.01)
