---
title: "RSV hospitalization burden"
author: "ZHE ZHENG"
date: "3/8/2021"
output: html_document
---

## Load necessary packages

```{r}
library(rjags)
```

## Time-series model structure
```{r}
library(rjags)

model_string<-"
model {

for(k in 1:n.group){  
  for (j in 1:n.age){
    for(n in 1:n.state){
      for (i in 1:n.date) { 
    
   lambda[i,j,k,n] <-   rd0[j,k,n]+
               epi[epi.year[i],j,k,n] +
               delta[month[i],j,k,n] +
               rsv[i,j,k,n]*rd2[j,k,n] + 
               flu[i,j,k,n]*rd1[epi.year[i],j,k,n]+
               phi[i,j,k,n]

  y[i,j,k,n] ~ dpois(lambda[i,j,k,n])  ## likelihood 

  phi[i,j,k,n] ~ dnorm(0,inv_var)

}

  # baseline hospitalizations (must be greater than or equal to 0)
  rd0[j,k,n] <- exp(beta0[j,k,n])
  beta0[j,k,n]~ dnorm(mu0,tau0)
  
  for (p in 1:n.year) { 
    # epi-year effects
    epi[p,j,k,n] ~ dnorm(xi_epi[p], tau.epi) # random slope

    # coefficient of influenza-associated respiratory hospitalization
    # this coefficient varies annually
    rd1[p,j,k,n] <- exp(beta1[p,j,k,n]) # ensure positive coefs
    beta1[p,j,k,n] ~ dnorm(beta1_mean[p,j,k,n],tau.flu)
    beta1_mean[p,j,k,n] <- gamma_flu[k]+ omega_flu[j]+ zeta_flu[n]+ xi_flu[p]
    }
  
    # coefficient of RSV-associated respiratory hospitalization
    # this coefficient depends on urbanisity, age and the belonging state
    rd2[j,k,n] <- exp(beta2[j,k,n]) # ensure positive
    beta2[j,k,n] ~ dnorm(beta2_mean[j,k,n],tau.rsv)
    beta2_mean[j,k,n]<- gamma[k]+omega[j]+zeta[n]
    
  
  # Month dummy variable, account for other pathogens and general seasonality 
  for (m in 1:12){
    delta[m,j,k,n] ~ dnorm(me,disp.m)
}}}}


## hyperpriors

for(n in 1:n.state){
zeta[n] ~ dnorm(0,tau1)
zeta_flu[n] ~ dnorm(0,tau2)
}

for(k in 1:n.group){
gamma_flu[k] ~ dnorm(0,tau3)
gamma[k] ~ dnorm(0, tau4)
}

for(j in 1:n.age){
omega[j] ~ dnorm(0, tau5)
omega_flu[j] ~ dnorm(0,tau6)
}

for (p in 1:n.year) { 
xi_flu[p] ~ dnorm(0,tau7)
xi_epi[p] ~ dnorm(0,tau8)
}
  inv_var ~ dgamma(0.01,0.01)
  mu0 ~ dnorm(0,0.0001)
  me ~ dnorm(0, 0.001)
  tau0 ~ dgamma(0.01, 0.01)
  tau1 ~ dgamma(0.01, 0.01)
  tau2 ~ dgamma(0.01, 0.01)
  tau3 ~ dgamma(0.01, 0.01)
  tau4 ~ dgamma(0.01, 0.01)
  tau5 ~ dgamma(0.01, 0.01)
  tau6 ~ dgamma(0.01, 0.01)
  tau7 ~ dgamma(0.01, 0.01)
  tau8 ~ dgamma(0.01, 0.01)
  tau.epi ~ dgamma(0.01, 0.01)
  tau.flu ~ dgamma(0.01, 0.01)
  tau.rsv ~ dgamma(0.01, 0.01)
  disp.m ~ dgamma(0.01, 0.01)
}
"
```


## Posterior sampling
```{r}
epi.year <- as.factor(rep(1:9, each=12))
month <- rep(1:12,9)

dataset <- list('y' = y, "rsv"=rsv,"flu"=flu, 'epi.year'=epi.year,"month"=month,n.age=9,n.date=108,n.year=9,"n.group"=3,n.state=3)

jags_post <- jags.model(textConnection(model_string), data = dataset,
                    n.chains = 2)

 update(jags_post, 
         n.iter=50000)


  rsv_resp <- coda.samples(jags_post, variable.names=c("rd2","lambda","rd1","phi", "rd0","epi","delta"),
                      thin = 20,n.iter = 40000)
  
  gelman.diag(rsv_resp[,1])
  gelman.diag(rsv_resp[,200])
  gelman.diag(rsv_resp[,500])
  gelman.diag(rsv_resp[,700])
  gelman.diag(rsv_resp[,1000])
  gelman.diag(rsv_resp[,1800])
  gelman.diag(rsv_resp[,2800])
  gelman.diag(rsv_resp[,3800])
  gelman.diag(rsv_resp[,5800])
  gelman.diag(rsv_resp[,7800])
  gelman.diag(rsv_resp[,9800])
  gelman.diag(rsv_resp[,11800])
  gelman.diag(rsv_resp[,13800])
  gelman.diag(rsv_resp[,15800])
  gelman.diag(rsv_resp[,17800])
  gelman.diag(rsv_resp[,19800])
```

## Calculate RSV attributable respiratory hospitalization incidence and percent
```{r}
  post <- as.data.frame(as.matrix(rsv_resp))
  lambda <- post[, grep("lambda", colnames(post), fixed=T)]
  rd2.resp <- post[, grep("rd2[", colnames(post), fixed=T)]
  
  rsv_count_nj <- array(data = NA,dim = c(4000,108,9,3))
for (i in 1:108) {
  for (j in 1:9) {
    for (k in 1:3) {
      rsv_count_nj[,i,j,k] <- rsv[i,j,k,1]*rd2.resp[,j+9*(k-1)]
    }}}

rsv_count_ny <- array(data = NA,dim = c(4000,108,9,3))
for (i in 1:108) {
  for (j in 1:9) {
    for (k in 1:3) {
      rsv_count_ny[,i,j,k] <- rsv[i,j,k,2]*rd2.resp_2[,j+9*(k-1)+27]
    }}}

rsv_count_wa <- array(data = NA,dim = c(4000,108,9,3))
for (i in 1:108) {
  for (j in 1:9) {
    for (k in 1:3) {
      rsv_count_wa[,i,j,k] <- rsv[i,j,k,3]*rd2.resp[,j+9*(k-1)+54]
    }}}

  rsv_count_nj_age <- array(data = NA,dim = c(4000,9,3))
  for (i in 1:4000) {    
    for (j in 1:9) {
      for (k in 1:3) {
       rsv_count_nj_age[i,j,k] <- sum(rsv_count_nj[i,,j,k])}}}
  
  rsv_count_ny_age <- array(data = NA,dim = c(4000,9,3))
      for (i in 1:4000) {
      for (j in 1:9) {
      for (k in 1:3) {
       rsv_count_ny_age[i,j,k] <- sum(rsv_count_ny[i,,j,k])}}}


rsv_percent_nj <- array(data = NA,dim = c(4000,9,3))
for (j in 1:9) {
  for (k in 1:3) {
    rsv_percent_nj[,j,k] <- as.numeric(rowSums(rsv_count_nj[,,j,k])/rowSums(lambda[,((j-1)*108+(k-1)*108*9+1):((j-1)*108+(k-1)*108*9+108)]))
  }}

rsv_percent_ny <- array(data = NA,dim = c(4000,9,3))
for (j in 1:9) {
  for (k in 1:3) {
    rsv_percent_ny[,j,k] <- as.numeric(rowSums(rsv_count_ny[,,j,k])/rowSums(lambda[,((j-1)*108+(k-1)*108*9+1+2916):((j-1)*108+(k-1)*108*9+108+2916)]))
  }}

rsv_percent_wa <- array(data = NA,dim = c(4000,9,3))
for (j in 1:9) {
  for (k in 1:3) {
    rsv_percent_wa[,j,k] <- as.numeric(rowSums(rsv_count_wa[,,j,k])/rowSums(lambda[,((j-1)*108+(k-1)*108*9+1+5832):((j-1)*108+(k-1)*108*9+108+5832)]))
  }}

resp_nj <- array(data = NA,dim = c(4000,9,3))
for (j in 1:9) {
  for (k in 1:3) {
    resp_nj[,j,k] <- as.numeric(rowSums(lambda[,((j-1)*108+(k-1)*108*9+1):((j-1)*108+(k-1)*108*9+108)]))
  }}

resp_ny <- array(data = NA,dim = c(4000,9,3))
for (j in 1:9) {
  for (k in 1:3) {
    resp_ny[,j,k] <- as.numeric(rowSums(lambda[,((j-1)*108+(k-1)*108*9+1+2916):((j-1)*108+(k-1)*108*9+108+2916)]))
  }}

resp_wa <- array(data = NA,dim = c(4000,9,3))
for (j in 1:9) {
  for (k in 1:3) {
    resp_wa[,j,k] <- as.numeric(rowSums(lambda[,((j-1)*108+(k-1)*108*9+1+5832):((j-1)*108+(k-1)*108*9+108+5832)]))
  }}
```

```{r}
round(apply(rsv_percent_ny,c(2,3), mean)*100,1)
round(apply(rsv_percent_ny, c(2,3), quantile , probs = 0.025)*100,1)
round(apply(rsv_percent_ny, c(2,3), quantile , probs = 0.975)*100,1)
```

